...
matrix = Matrix.rows(raw.lines.map { |l| l.split(' ').map(&:to_i) })

#Horizontal / Vertical
for x in 0..19
	for y in 0..19
		product = matrix.row(x)[y..y+3].map{|val| product *= val}.last
		max = product if product > max
		product = 1
		product = matrix.column(x)[y..y+3].map{|val| product *= val}.last
		max = product if product > max
		product = 1
	end
end
